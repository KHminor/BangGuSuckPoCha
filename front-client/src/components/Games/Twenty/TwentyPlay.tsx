    ...
    <>
     <div className={`${styles.setSize}`}>
        <div className={`${styles.title}`}>
            YES OR NO <img src= {require("./TwentyImg/free-icon-mountain-5318286.png")} className={`${styles.image}`}/>
        </div>
        ...
        <>  
            ...
            {pochaUsers.map(function(data:any) {
                return (
                    <>
                        <div key={data.nickname} className={`${styles.subjects}`} onClick={() => selectPresenter(data.username, data.nickname)}>
                             <div className={`${styles.hiddenBox}`}><img src ={require("./TwentyImg/yield.png")} className={`${styles.subjectsArrow}`} /></div>
        ...
        <>
            ...
            <div  className={'text-base p-1 text-white'}>출제자 : {presenterNickname}</div>
            {subjectList.map(function(data) {
                return (
                    <>
                    {myName === presenter ? 
                    <>
                        <div key={data} className={`${styles.subjects}`} onClick={() => subjectSelect(data)}>
                            <div className={`${styles.hiddenBox}`}><img src ={require("./TwentyImg/yield.png")} className={`${styles.subjectsArrow}`} /></div>
                            <div>{data}</div>
                         </div>
                    </> 
        ...
        <>
            { myName === presenter ? 
                    <>
                        ...
                        <div><input id="answer" className={`${styles.answerInput}`} type="text" onChange={(e) => {setAnswer(e.target.value)}}/></div>
        ...
        {signal === "START" ?
        <>
            <div className={`${styles.detail}`}>질문을 통해 정답을 맞춰보세요!</div>
            <div className={`${styles.subject}`}>
                {subject}
                    <div className={`${styles.countDetail}`} style={{color:"white"}} > 남은 질문 횟수</div>
                    <div className={'text-8xl p-1 text-white'}>{num}<span className={'text-5xl'}>회</span></div>
                <div className={`${styles.text}`}>{ox}</div>
            </div>
            { myName === presenter ? 
            <> 
                <div className={`${styles.buttons}`}>
                    <div className={`${styles.button}`} onClick={correctanswer}>정답</div>
                    <div className={`${styles.button}`} onClick={correct}>O</div>
                    <div className={`${styles.button}`} onClick={incorrect}>X</div>
                </div>
            </> : null}
        ...
        <div  className={`${styles.reset}`}>
            ...
            <div className={`${styles.button}`} onClick = {exit} > EXIT </div>
        </div> 
        </> : null}
