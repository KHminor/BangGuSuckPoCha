  ...
  const friendList = menuFriendListApiData.map((e:any, idx:any)=> {
    ...
    return (
      <div key={idx} className=" grid my-2 cursor-pointer " style={{gridTemplateColumns: '1fr 3fr 1fr'}} onClick={()=> {
        ...
        <div className={`flex justify-start items-center pl-3 text-base font-semibold h-full ${styles.menuFriendNeon}`}>{e.f_nickname}</div>
      ...
      <div className="h-full w-full" >
        <div className="w-full h-full">
          <div className="grid h-full bg-black text-white border-2 border-white" style={{gridTemplateRows: '0.5fr 0.5fr 5fr', borderRadius: '24px'}}>
            <div className="grid" style={{gridTemplateColumns: '2fr 1.2fr 1fr 1fr'}}>
              <div></div>
              <div className='flex justify-center items-center h-full'>친구목록</div>
              <div></div>
              {/* 친구 리스트 및 채팅창 닫기 */}
            ...
            <div className="flex justify-between items-center rounded-full bg-white h-[80%] border-2 border-stone-400">
              <input className="w-[84%] h-full text-base text-black font-bold pl-3 " style={{borderRadius: '100% 0px 0px 100%'}}  type="text" />
              <div className='w-[5%]'></div>
              <div className='w-[11%] cursor-pointer'>
                <img className='w-[1rem] h-[1rem]' src={require('../../assets/mainIcon/search.png')} alt="" />
              </div>
            </div>
